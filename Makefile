...
GO_FORMAT_DIRS := cmd internal schemas shell .github/scripts/test-audit .github/scripts/artifact-qualify .github/scripts/coverage-check .github/scripts/provider-capability-probe .github/scripts/govuln-summary
...
ssh-test:
	...
	@test "$$INTENT_SH_TEST_SSH_LOOPBACK" = 1 || { echo "ssh-test requires the job-owned loopback SSH fixture; use external-ssh-test for a caller-owned target" >&2; exit 1; }
	INTENT_SH_CI_STRICT=1 go test -json ./internal/shelltest -run '$(SSH_RUN)' -count=1 | $(AUDITOR) -suite ssh-loopback $(AUDIT_MATRIX) $(call AUDIT_OUTPUT,ssh-loopback)
...
external-ssh-test:
	@test -n "$$INTENT_SH_TEST_SSH_TARGET" || { echo "INTENT_SH_TEST_SSH_TARGET is required" >&2; exit 1; }
	env -u INTENT_SH_TEST_SSH_CONFIG -u INTENT_SH_TEST_SSH_LOOPBACK -u INTENT_SH_TEST_BINARY -u INTENT_SH_COVERAGE_DIR -u INTENT_SH_EXEC_COVERAGE_DIR \
...
coverage-test:
	...
	@test -n "$$INTENT_SH_TEST_SSH_TARGET" || { echo "loopback SSH target is required for executable coverage" >&2; exit 1; }
	@test "$$INTENT_SH_TEST_SSH_LOOPBACK" = 1 || { echo "executable coverage requires the job-owned loopback SSH fixture" >&2; exit 1; }
	...
	INTENT_SH_EXEC_COVERAGE_DIR="$$COVERAGE_DIR/executable" INTENT_SH_TEST_BINARY="$$COVERAGE_DIR/intent-sh" INTENT_SH_COVERAGE_DIR="$$COVERAGE_DIR/executable" $(MAKE) ssh-test QUALIFICATION_DIR="$$COVERAGE_DIR/results/ssh"
	...
	for target_os in darwin linux; do \
		for target_arch in amd64 arm64; do \
			CGO_ENABLED=0 GOOS="$$target_os" GOARCH="$$target_arch" \
				go build -trimpath -o "$$temporary/intent-sh-$$target_os-$$target_arch" ./cmd/intent-sh || exit; \
		done; \
	done

static-check: ci-tools fmt-check vet module-check shell-check shell-lint workflow-lint openspec-check supported-builds
